...
const DIAG = `Verify a metrology design decision. Context: the shadow gauge (tools/measure-shadow.mjs) measures in ?metro=1 (jitter/accum/grain/pincushion disabled; bloom-off, ultra-tier, HDR asserted). Read tools/measure-shadow.mjs and the render() function in src/main.js and answer: (1) does the metro path cleanly bypass jitter+accumulation with no residual coupling? (2) is the 50%-of-plateau edge estimator unbiased for the hard-step edges metro produces? (3) any remaining contamination path from the presentation pipeline? Also audit tools/science-suite.mjs: does its JS mirror of trace() (same RK4, same dt schedule, same classification) faithfully match src/shaders.js? Report problems as findings (shot:'code'); if sound, clean=true with empty findings. Then StructuredOutput — that call is your entire answer. No prose.`

const PHYS_C = `Numerical-analysis audit. Read /Users/mohammedhossam/blackhole/src/shaders.js (trace function) and /Users/mohammedhossam/blackhole/tools/science-suite.mjs. Attack the NUMERICS: (1) the RK4 step on u'' = 3Mu^2 - u with adaptive dphi = dt*0.09*clamp(0.35/u, 0.6, 1.0) — stability and error ordering near the photon sphere where the separatrix is exponentially sensitive; (2) the suite claims 4th-order convergence (measured ratio 17.2) and invariant drift 1e-7 — are those thresholds honestly derived or fitted?; (3) the secant plane-crossing iteration — contraction guaranteed? any bisection-branch pathological case?; (4) float32 GPU vs float64 suite mirror — can the GPU's shorter mantissa shift the capture boundary enough to matter at the 0.5% criterion?; (5) the escape-sphere interpolation f-clamp — error order? Report violations as findings (shot:'code'). Then StructuredOutput — that call is your entire answer. No prose.`

const PHYS_D = `HOSTILE FALSIFICATION REVIEW. Your job is to prove this renderer LIES about physics. Read /Users/mohammedhossam/blackhole/src/shaders.js fully and /Users/mohammedhossam/blackhole/SCIENTIFIC_AUDIT.md. Hunt for: any visual the renderer produces that a physicist would call misleading while the audit claims exactness; any equation that is invented, mis-signed, or dimensionally wrong; any constant that is visually convenient rather than derived; any place where an artistic choice silently leaks into the science path (uScience > 0.5 branches); any claim in SCIENTIFIC_AUDIT.md not actually supported by the code. The audit's honesty is your target — falsify it. Report each successful falsification as a finding (shot:'code', severity by how much it misleads). If you cannot falsify anything material, clean=true. Then StructuredOutput — that call is your entire answer. No prose.`
...
const physTasks = [
  ...
  { kind:'phys', id:'numerical', prompt: PHYS_C, schema: FIND_SCHEMA },
  { kind:'phys', id:'hostile', prompt: PHYS_D, schema: FIND_SCHEMA },
